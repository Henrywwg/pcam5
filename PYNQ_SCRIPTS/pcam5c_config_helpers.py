...
#Written by F. Wysong w/ minor debugging from gemini (went insane over a tuple error)


# DISCLAIMER DISCLAIMER DISCLAIMER ALL REGISTER MAPPINGS (Middle couple hundred lines) ARE DERIVED FROM DIGILENT REPO FOUND BELOW
...
# maybe you can learn some more about the registers here but I don't have the time to write now... good luck :P
...
# NOTE: This function written by Gemini (note the reliance on globals. my functions primarily use params to help differentiate)
# If your stuff is named differently, ditch this or rename as needed.
...
# configure standard resolution - more maybe coming
...
def _PCAM_PC(pwup_pin):
    ...
    pwup_pin.write(0) 
    time.sleep(0.1)     # Need to make sure that the camera actually has time to settle.
...
def _PCAM_START_CAPTURE():
    ...
    time.sleep(0.5)
    print("PCAM5 started. Delayed to allow settling", flush=True)    
...
def _VDMA_CONFIG_WHOLISTIC(ol_id, frame_buffer, width, height):
    ...
    while (ol_id.read(0x30) & 0x04) == 0x04:
        ...
        timeout -= 1
        if timeout == 0:
            print("ERR: VDMA Reset stuck")
            break

    print("Reset settled.")
    ol_id.write(0x30, 0x3)        # Start VDMA as writer

    # NOTE/IMPORTANT VDAM is configured for 4 frame buffers in my version. adjust as needed
    ...
    print(f"VDMA status: {hex(status)}", flush=True)
    if status == 0x10000:
        print("VDMA configured and waiting :D", flush=True)
...
def _ALLOCATE_FRAME_BUFFER(width, height):
    ...
    # if you see gray the frame buffer isn't being written to. Useful for debugging but it ultimately unnecessary in productino
    print("Frame buffer allocated of requested size. Clearing buffer to gray for debugging.")
    fb[:] = 128
    fb.flush()
    print(f"Physical address: {hex(fb.device_address)}\n", flush=True)
    return fb
